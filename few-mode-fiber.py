...
def get_hrw(ts, dt, t1=12.2e-3, t2=32e-3):
    """
    Returns:
      hr: Raw response function h(t) [units: 1/ps] -> 그래프용
      h_discrete: Discrete kernel for simulation (sum=1) -> 시뮬레이션용
      hrw: Frequency domain response -> 시뮬레이션용
    """
    # 1. Analytic Response Function (문헌값 비교용)
    # 0으로 나누기 방지 및 t<0 영역 처리 (ts가 0부터 시작하므로 그대로 진행)
    hr = ((t1**2 + t2**2) / (t1 * t2**2)) * np.sin(ts/t1) * np.exp(-ts/t2)
    
    # 2. Discrete Kernel (시뮬레이션 용)
    h_discrete = hr * dt
    
    # 수치적 오차 보정을 위한 정규화 (선택 사항이나 시뮬레이션엔 권장)
    norm_factor = np.sum(h_discrete)
    if norm_factor > 0:
        h_discrete /= norm_factor
        # 주의: hr(그래프용)은 물리적 의미 보존을 위해 보통 재정규화 하지 않거나,
        # 꼭 해야 한다면 hr /= norm_factor 로 똑같이 맞춰줍니다.
    
    hrw = np.fft.fft(h_discrete)
    return hr, h_discrete, hrw

# 설정
dt = 0.001 # 1 fs
Nt = 2**13
ts = np.arange(Nt) * dt # ps 단위

# 계산
hr, h_disc, hrw = get_hrw(ts, dt)

# 
# 그래프 그리기
plt.figure(figsize=(10, 6))
plt.plot(ts * 1000, hr, label='My Code (hr)') # x축 fs로 변환
plt.xlim(0, 800) # 보통 0~300fs 사이에서 반응이 끝남
plt.xlabel('Time (fs)')
plt.ylabel('Response Function $h_R(t)$ ($ps^{-1}$)')
plt.title('Raman Response Function for Silica')
